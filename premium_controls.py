...
def normalize_premium_controls(settings: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if caption_mode not in {"full", "compact"}:
        caption_mode = "full"
    language = settings.get("twitter_language")
...
async def save_premium_controls(db, guild_id: int, settings: Mapping[str, Any]) -> None:
    ...
    await db.execute(
        """INSERT INTO guild_premium_controls (
            ...
            twitter_language, ignored_user_ids, ignored_role_ids
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(guild_id) DO UPDATE SET
            ...
            twitter_language = excluded.twitter_language,
...
async def load_premium_controls(db) -> dict[int, dict[str, Any]]:
    cursor = await db.execute(
        ...
        card_caption_mode, twitter_language, ignored_user_ids, ignored_role_ids
...
def resolve_twitter_language(
    ...
    *,
    ...
    if not premium:
        return None
    return normalize_premium_controls(settings)["twitter_language"]
